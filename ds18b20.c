...
static uint8_t m_converting=0;
static uint8_t m_wait=0;
...
float Get_Current_Temper(void){
	return temper_sensor.Temperature;
}
uint8_t Return_Init(void){
	return m_init;
}
uint8_t Return_waiting(void){
	//return m_wait;
	return Waiting_Onewire();
}
uint8_t Return_Converting(void){
	return m_converting;
}
...
bool Temperature_Sensor_Init(void){
	...
	//onewire address setting
	OneWire.ROM_NO[0]=0x28;
	OneWire.ROM_NO[1]=0x36;
	OneWire.ROM_NO[2]=0x56;
	OneWire.ROM_NO[3]=0x94;
	OneWire.ROM_NO[4]=0x97;
	OneWire.ROM_NO[5]=0x0d;
	OneWire.ROM_NO[6]=0x03;
	OneWire.ROM_NO[7]=0x9e;

	OneWire_GetFullROM(&OneWire,temper_sensor.Address);
	Ds18b20Delay(50);
	DS18B20_SetResolution(&OneWire,temper_sensor.Address, DS18B20_Resolution_12bits);
	Ds18b20Delay(50);
	DS18B20_DisableAlarmTemperature(&OneWire,temper_sensor.Address);

	m_init=1; // init finish
	return true;
}
...
void Check_Converting(void){
	//m_wait=1;
	m_converting=!DS18B20_AllDone(&OneWire);
	//m_wait=0;
	// if converting finish -> DS18B20_AllDone(&OneWire)=1
	// -> m_converting=0
}
...
float Get_Temper(void){
	Ds18b20Delay(100);
	//m_wait=1;
	temper_sensor.DataIsValid = DS18B20_Read(&OneWire,temper_sensor.Address,&temper_sensor.Temperature);
	//m_wait=0;
	return temper_sensor.Temperature;
}
